...
const shopApi = {
    ...
    getSuggestions: (keyword, options = {}, token) => {
        ...
        return fetch(`${process.env.REACT_APP_URL_SIPLAH}product/search?${qs.stringify(params)}`, {
            method: 'get',
            headers: {
                Authorization: token || ''
            }
        ...
        return fetch(`${process.env.REACT_APP_URL_SIPLAH}backendseller/paket/paket_api/detailPacket/${id}`).then(res => res.json())
    ...
    getProductReviews: (productId, options = { page: 1, limit: 10 }) => {
        ...
        return fetch(`${process.env.REACT_APP_URL_SIPLAH}product/review/${productId}?${qs.stringify(params)}`).then((res) => res.json());
    },
    getMallReviews: (mallId, options = { page: 1, limit: 1 }) => {
        const params = { ...options }
        return fetch(`${process.env.REACT_APP_URL_SIPLAH}mall/review/${mallId}?${qs.stringify(params)}`).then((res) => res.json());
    },
}
