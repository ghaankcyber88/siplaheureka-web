...
function ShopPageSearch(props) {
    ...
    useEffect(() => {
        ...
        shopApi.getProductSearch(state.keyword, state.options, { ...state.filters }, customer.token).then((result) => {
            const { data } = result;
            if (canceled) {
                return;
            }
        ...
        <CategorySidebar offcanvas={offcanvas}>
            <CategorySidebarItem>
                <WidgetFilters
                    title="Filter"
                    offcanvas={offcanvas}
                    filters={state.productsList.filters.map(item => item.type === 'category' ? ({ ...item, selected: state.options.categoryId }) : ({ ...item }))}
